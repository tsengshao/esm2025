...
DO fileid=1,n_fields+1
  ...
  ! read netcdf file 
  CALL read_single_nc('TC.nc','TC',previoustimestep+1,input_field(:,:,fileid))
...
SUBROUTINE set_event_number_to_value(domainsize_x,domainsize_y,value,event_number,nevent, &
  ...
  USE irt_parameters, ONLY: lperiodic_x, lperiodic_y, lpole
  ...
  if( y2 < y1 .and. lpole) then
    x2 = x2 + domainsize_x / 2
    y2 = y2 + domainsize_y
  endif
...
do while( stack_num .GT. 0)
  ...
  do imove=1,nmove
    ...
    if (lpole .and. (locy2>domainsize_y .or. locy2<1)) then
      iox = MODULO(locx2 + domainsize_x / 2, domainsize_x)+1
      ioy = locy
    else
    ...
    if((.not. lperiodic_y) .and. (.not. lpole) .and. &
